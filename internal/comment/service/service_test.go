...
type fakeRepository struct {
	items map[string]*entity.Comment
}

func newFakeRepository() *fakeRepository {
	return &fakeRepository{items: make(map[string]*entity.Comment)}
}

func (r *fakeRepository) Save(ctx context.Context, item *entity.Comment) error {
	copy := *item
	r.items[item.ID] = &copy
	return nil
}

func (r *fakeRepository) FindByID(ctx context.Context, id string) (*entity.Comment, error) {
	item, ok := r.items[id]
	if !ok {
...
func (r *fakeRepository) List(ctx context.Context, offset int, limit int) ([]*entity.Comment, int64, error) {
	items := make([]*entity.Comment, 0, len(r.items))
