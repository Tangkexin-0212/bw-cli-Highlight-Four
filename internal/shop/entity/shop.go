...
var (
	...
	ErrUserNotAllowed   = errors.New("用户状态异常")
	ErrProductNotOnSale = errors.New("商品已下架")
)

// Shop 是 shop 业务服务的聚合根。
...
func (item *Shop) Update(name string, description string) error {
	name = strings.TrimSpace(name)
	description = strings.TrimSpace(description)
...
type Product struct { //商品表
	...
	Status      int64   `gorm:"type:int(1)"`        //商品状态 1上架 2下架 3售牂 4审核中 5审核不通过
	CreatedAt   *time.Time
}
