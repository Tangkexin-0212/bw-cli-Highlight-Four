...
func TestServiceCRUD(t *testing.T) {
	...
	require.ErrorIs(t, err, entity.ErrShopNotFound)
}

type fakeRepository struct {
	items map[string]*entity.Shop
}

func newFakeRepository() *fakeRepository {
	return &fakeRepository{items: make(map[string]*entity.Shop)}
}

func (r *fakeRepository) Save(ctx context.Context, item *entity.Shop) error {
	copy := *item
	r.items[item.ID] = &copy
	return nil
}

func (r *fakeRepository) FindByID(ctx context.Context, id string) (*entity.Shop, error) {
	item, ok := r.items[id]
	if !ok {
		return nil, entity.ErrShopNotFound
	}
	copy := *item
	return &copy, nil
}

func (r *fakeRepository) List(ctx context.Context, offset int, limit int) ([]*entity.Shop, int64, error) {
	items := make([]*entity.Shop, 0, len(r.items))
